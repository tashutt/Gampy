...
X1, Y1 = np.histogram(background_data.query('good == 1')['energy_from_sum'], bins=15, range=(EMIN, 10_500))
X2, Y2 = np.histogram(background_data.query('good == 1 and beta_minus_Dbeta < @ACCEPTANCE_ANGLE')['energy_from_sum'], 
                      bins=15, range=(EMIN, 10_500))
background_ratio = X2/X1
pos_mask = background_ratio > -1e-12


def electron_track_rejection(x, a, b, c, d):
    return a*np.exp(-b*x) + c*np.log(x) + d +0.01
    
etr_popt, _ = curve_fit(electron_track_rejection, 
                        Y1[:-1][pos_mask], 
                        background_ratio[pos_mask],
                        p0=[1, 1e-3, 1e-2, 1])
...
etr_background  = base_background * np.maximum(electron_track_rejection(E*1000, *etr_popt),1e-3)
